...
function getPlaceReveal(surah) {
  if (surah.tempatTurun && (surah.tempatTurun === "Mekah" || surah.tempatTurun === "Madinah"))
    return surah.tempatTurun;
...
function playFullSurah() {
  ...
  currentAudioObj = audio;
  const nowPlayingSpan = document.getElementById('nowPlayingStatus');
  if (nowPlayingSpan) nowPlayingSpan.innerText = `🎧 Memutar full surah (${RECITER_MAP[currentReciter] || currentReciter}) tanpa jeda`;
  ...
  audio.onended = () => {
    ...
    if (nowPlayingSpan) nowPlayingSpan.innerText = '✅ Selesai';
  };
  audio.onerror = () => {
    if (currentAudioObj === audio) currentAudioObj = null;
  ...
  try {
    ...
    if (availableReciters.length > 0) {
      qariOptionsHtml = `
        <div class="qari-selector">
          ...
          <select id="qariSelect">
            ${availableReciters.map(key => `<option value="${key}" ${key === currentReciter ? 'selected' : ''}>${RECITER_MAP[key] || key}</option>`).join('')}
          </select>
        </div>
      `;
    } else {
      qariOptionsHtml = `
        <div class="qari-selector">
          <label>🎙️ Qari:</label>
          <select id="qariSelect">
            ${Object.entries(RECITER_MAP).map(([key, name]) => `<option value="${key}" ${key === currentReciter ? 'selected' : ''}>${name}</option>`).join('')}
          </select>
        </div>
      `;
    }
    ...
    for (let ayat of ayatArray) {
      ...
      if (hasTafsir) {
        ...
        tafsirPreviewHtml = `<div class="tafsir-preview">📖 Tafsir: (Tidak tersedia untuk ayat ini)</div>`;
      }
    ...
    const fullHtml = `
      <div class="detail-header">
        ...
        <div class="audio-controls">
          <button id="playSurahBtn" class="play-surah-btn">▶ Putar Surah (Full, tanpa jeda)</button>
    ...
    if (playBtn) playBtn.addEventListener('click', (e) => { e.stopPropagation(); playFullSurah(); });
...
surahContainer.addEventListener('click', (e) => {
  ...
  if (card && !isLoadingDetail) {
    loadSurahDetail(parseInt(card.dataset.surahId));
  }
});
